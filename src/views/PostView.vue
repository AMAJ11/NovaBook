...
<v-col cols="12" class="bg-primary hidden-sm-and-down mt-8" style="flex-wrap: wrap;display: flex;justify-content: space-between;">
      ...
      <v-row class="mt-5" style="max-width:100%">
         ...
         <v-col cols="12" md="8">
            <v-row class="pa-8 pt-4 px-lg-16 px-md-16 px-sm-8 px-3  mb-6" style="">
               <v-col cols="12" v-for="i in this.posts">
                  <v-card elevation="0" variant="solo" class="pa-2 pa-sm-5 " style="width: 100% !important;">
                     ...
                     <v-card-actions calss="" style="height:15px;display:flex;justify-content: space-between;">
                        ...
                        <v-btn size="large" class="hidden-sm-and-down" @click="this.comment = true" color=""
                           append-icon="mdi-message-alert-outline">Comment</v-btn>
                        <v-btn size="large" class="hidden-sm-and-down" append-icon="mdi-thumb-up-outline" @click="addLike(i.id)">Like {{ i.likes.length }}</v-btn>

                        <v-btn size="large" class="hidden-md-and-up"
                        ...
                        <v-btn size="large" class="hidden-md-and-up"
                           @click="addLike(i.id)" :loading="this.WillLike" :color="islike(i.id)? 'blue':''"><v-icon>mdi-thumb-up-outline</v-icon>
                           <p> {{ i.likes.length }} </p>
                        </v-btn>
                     </v-card-actions>
                     <v-divider></v-divider>
      ...
      <V-dialog style="height:400px;max-width:700px" v-model="this.comment">
            ...
            <div style="min-height:100%">
               <v-row class="pa-9 pb-16">
                  <v-col class="mb-5" style="display: flex;flex-direction: column;" cols="12" v-for="i in 14">
                     ...
                     <span style="display: flex;"><v-avatar> <v-img src="../assets/logo.png"
                              width="150px"></v-img></v-avatar>
                        <h3>Ammar AJ</h3>
                     </span>
                     <p> {{ i }} Lorem, ipsum dolor sit amet consectetur adipisicing elit. Totam unde accusamus deleniti
                        nesciunt, ab, cumque porro ut, dolorum sapiente aliquid laborum praesentium? Sed iure dolorem
         ...
         <div style="display: flex;" class="mt-4">
            ...
            <v-btn style="translate: 0 10px" color="warning" variant="flat">Comment</v-btn>
         </div>
      </V-dialog>
   </v-app>
