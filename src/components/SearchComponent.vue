<template>
  <v-container>
    <v-text-field v-model="searchQuery" :label="lan == '1' ? 'اكتشف اصدقاء جدد' : 'Discover the world around you'" outlined
      @input="onSearchInput" @keyup.enter="searchUsers" :loading="isLoading"
      prepend-inner-icon="mdi-magnify"></v-text-field>

    <v-list v-if="searchResults.length > 0">
      <v-list-item v-for="user in searchResults" :key="user._id" @click="goToProfile(user._id)">
        <v-list-item-avatar>
